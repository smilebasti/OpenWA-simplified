...
// whatsapp-web.js does not ship channel types; use an opaque interface
interface WwjsChannelClient {
  sendMessage(
    content: string | MessageMedia,
    options?: { caption?: string },
  ): Promise<{ id: { _serialized: string }; timestamp: number }>;
}
...
export class WhatsAppWebJsAdapter extends EventEmitter implements IWhatsAppEngine {
  ...
  private channelCache: { data: Channel[]; expiresAt: number } | null = null;
  ...
  async initialize(callbacks: EngineEventCallbacks): Promise<void> {
    ...
    try {
      ...
      // Remove Chromium profile lock left by a previous container or crash.
      // LocalAuth stores the profile at <dataPath>/session-<clientId> directly
      // (no .wwebjs_auth subdirectory when a custom dataPath is supplied).
      const profileDir = path.resolve(
        this.config.sessionDataPath,
        `session-${this.config.sessionId}`,
      );
      this.logger.log(`Cleaning Chromium locks in: ${profileDir}`);
  ...
  async getSubscribedChannels(): Promise<Channel[]> {
    ...
    // Cache for 5 minutes — getChannels() is a slow remote WA call
    ...
    const result: Channel[] = (channels ?? []).map((ch: WwjsChannelData) => ({
      id: String(typeof ch.id === 'object' ? ch.id._serialized : ch.id),
      name: String(ch.name || ''),
      description: ch.description ? String(ch.description) : undefined,
    }));

    this.channelCache = { data: result, expiresAt: Date.now() + 5 * 60 * 1000 };
    return result;
  }
  ...
  async getChannelById(channelId: string): Promise<Channel | null> {
    this.ensureReady();
    try {
      const ch = await this.client!.getChatById(channelId) as WwjsChannelData | undefined;
      if (!ch) return null;
      return {
        id: String(typeof ch.id === 'object' ? ch.id._serialized : ch.id),
        name: String(ch.name || ''),
  ...
  async subscribeToChannel(inviteCode: string): Promise<Channel> {
    this.ensureReady();
    const ch = await (this.client as unknown as BusinessClient).subscribeToChannel(inviteCode) as WwjsChannelData;
  ...
  async getChannelMessages(channelId: string, limit: number = 50): Promise<ChannelMessage[]> {
    this.ensureReady();
    try {
      const ch = await this.client!.getChatById(channelId) as unknown as { fetchMessages: (opts: { limit: number }) => Promise<{ id: { _serialized: string }; body: string; timestamp: number; hasMedia: boolean; mediaUrl?: string }[]> } | null;
      if (!ch) {
  ...
  async sendChannelMessage(channelId: string, text: string): Promise<MessageResult> {
    this.ensureReady();
    const msg = await this.client!.sendMessage(channelId, text) as { id: { _serialized: string }; timestamp: number };
    this.logger.log(`Sent text to channel ${channelId}`);
    return { id: String(msg.id._serialized), timestamp: Number(msg.timestamp) };
  }

  async sendChannelMediaMessage(channelId: string, mediaUrl: string, caption?: string): Promise<MessageResult> {
    this.ensureReady();
    const media = await MessageMedia.fromUrl(mediaUrl, { unsafeMime: true });
    const msg = await this.client!.sendMessage(channelId, media, caption ? { caption } : {}) as { id: { _serialized: string }; timestamp: number };
    this.logger.log(`Sent media to channel ${channelId}`);
    return { id: String(msg.id._serialized), timestamp: Number(msg.timestamp) };
  }
