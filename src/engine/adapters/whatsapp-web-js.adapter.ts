...
export class WhatsAppWebJsAdapter extends EventEmitter implements IWhatsAppEngine {
  ...
  async initialize(callbacks: EngineEventCallbacks): Promise<void> {
    ...
    try {
      ...
      for (const lock of ['SingletonLock', 'SingletonSocket', 'SingletonCookie']) {
        ...
        try { fs.unlinkSync(lockPath); } catch { /* not present, ignore */ }
      }
  ...
  async sendChannelMessage(channelId: string, text: string): Promise<MessageResult> {
    this.ensureReady();
    const ch = (await (this.client as unknown as BusinessClient).getChannelById(channelId)) as WwjsChannelClient | null;
    if (!ch) throw new Error(`Channel ${channelId} not found`);
    const msg = await ch.sendMessage(text);
  ...
  async sendChannelMediaMessage(channelId: string, mediaUrl: string, caption?: string): Promise<MessageResult> {
    ...
    const msg = await ch.sendMessage(media, caption ? { caption } : undefined);
