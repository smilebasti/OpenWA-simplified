...
export function Sessions() {
  ...
  const handleStart = async (id: string) => {
    ...
    try {
      ...
      // "already started" is expected when session auto-started on creation — not an error
      if (!(err instanceof Error && err.message.toLowerCase().includes('already started'))) {
        const msg = err instanceof Error ? err.message : t('sessions.create.errorDefault');
  ...
  const handleShowQR = async (id: string) => {
    ...
    // Retry up to 10 times with 2 s delay — QR may not be ready immediately after start
    for (let attempt = 0; attempt < 10; attempt++) {
      try {
        ...
        const notReady = err instanceof Error && err.message.toLowerCase().includes('not ready');
        ...
        <div className="modal-overlay" onClick={() => setShowCreateModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              {newSessionName && /^[a-z0-9-]+$/.test(newSessionName) && newSessionName.length < 3 && (
                <p className="input-error">{t('sessions.create.tooShort')}</p>
              )}
